...
export default function UpdatePatientFormPage() {
  ...
  async function fetchData() {
    ...
    // Convertir proprement la date
    const dob = data.date_naissance ? new Date(data.date_naissance) : undefined;
  ...
  async function onSubmit(data: z.infer<typeof newPatientSchema>) {
    ...
    if (data.age !== calculatedAge) {
        setMessage({ text: "Entered age does not match birth date.", type: "error" });
        return;
    }
  ...
  return (
    ...
    <Form {...form}>
      ...
      {message.text && (
        <div
          className={cn(
            "text-sm font-medium p-2 rounded-md w-70",
